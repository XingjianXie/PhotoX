export default {
    ...
    //Download
    getDownloadByPhotoId: 'SELECT * from download where photo = ?',

    //Message
    addMessage: 'INSERT INTO message(`from`, `to`, `content`) values(?, ?, ?)',
    queryMyMessageWithLimit: 'SELECT message.*, user.name AS from_name, `read`.message AS `read` FROM `message` LEFT OUTER JOIN user ON user.id=message.`from` LEFT OUTER JOIN `read` ON message.id=`read`.message WHERE (`to`=? OR `to` IS NULL) AND (read.user=? OR read.user IS NULL) LIMIT ?,?',
    countQueryMyMessageWithLimit: 'SELECT COUNT(*) FROM `message` LEFT OUTER JOIN user ON user.id=message.`from` LEFT OUTER JOIN `read` ON message.id=`read`.message WHERE (`to`=? OR `to` IS NULL) AND (read.user=? OR read.user IS NULL)',
    searchMyMessageWithLimit: 'SELECT message.*, user.name AS from_name, `read`.message AS `read` FROM `message` LEFT OUTER JOIN user ON user.id=message.`from` LEFT OUTER JOIN `read` ON message.id=`read`.message WHERE (`to`=? OR `to` IS NULL) AND (read.user=? OR read.user IS NULL) AND (POSITION(? IN user.name) OR POSITION(? IN message.`from`) OR POSITION(? IN `content`) OR POSITION(? IN message.id)) LIMIT ?,?',
    countSearchMyMessageWithLimit: 'SELECT COUNT(*) FROM `message` LEFT OUTER JOIN user ON user.id=message.`from` LEFT OUTER JOIN `read` ON message.id=`read`.message WHERE (`to`=? OR `to` IS NULL) AND (read.user=? OR read.user IS NULL) AND (POSITION(? IN user.name) OR POSITION(? IN message.`from`) OR POSITION(? IN `content`) OR POSITION(? IN message.id))',
